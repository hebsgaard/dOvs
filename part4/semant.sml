...
fun transExp (venv, tenv) =
    let
	  ...
	  (*  if !nesting > 0
	    ...
	    (err pos "You can't use the break expression"; {exp = (), ty=Ty.UNIT})
...
and transDec ( venv, tenv, A.VarDec {name, escape, typ = NONE, init, pos}) =
  ...
  | transDec (venv, tenv, A.TypeDec tydecls) =
    (case tydecls of 
	 ...
	 in
	     ( case S.look(tenv, sy) of
		   NONE =>  transDec (venv, tenv', A.TypeDec(tail))
		 | SOME _ => (err pos' ("Type already exists: " ^ S.name sy);  
  ...
  | transDec (venv, tenv, A.FunctionDec fundecls) = 
	...
	let 
	    ...
	    val venv' = 	 (case S.look(venv, name) of 
			    ...
			    | SOME _ => (err pos ("Function already exists: " ^ S.name name); venv))
	...
	in
	    ...
	    else (err pos ("Mismatch in return type in: " ^S.name name) ; {tenv = tenv, venv = venv'})
	end
    )
